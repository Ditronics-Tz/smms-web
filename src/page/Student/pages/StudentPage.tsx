...
import { Typography, Box, List, ListItem, ListItemContent, Chip, ColorPaletteProp, ListDivider, Sheet, Table, iconButtonClasses, Button, IconButton, Card, Input, ButtonGroup, Dropdown, MenuButton, Menu, Modal, ModalDialog, ModalClose, DialogTitle, DialogContent, FormControl, FormLabel, Stack, MenuItem, ListItemDecorator, Avatar } from "@mui/joy";
...
import { BlockOutlined, CalendarMonthOutlined, DeleteOutline, EditOutlined, FilterAltOutlined, PersonAddOutlined, RemoveRedEyeOutlined, WarningRounded } from "@mui/icons-material";
                    ...
                    >
                        <ListItemContent sx={{ display: 'flex', gap: 2, alignItems: 'start' }}>
                            ...
                            <div>
                                ...
                                <Typography level="body-xs" gutterBottom><b>{t("student.schoolName")}:</b> {listItem.school_name}</Typography>
                ...
                >
                    <thead>
                        <tr style={{ textAlign: 'center' }}>
                            ...
                            <th style={{ width: 100, padding: '10px 6px', }}>{t("student.firstName")}</th>
                            <th style={{ width: 100, padding: '10px 6px', }}>{t("student.middleName")}</th>
                            <th style={{ width: 100, padding: '10px 6px', }}>{t("student.lastName")}</th>
                            <th style={{ width: 100, padding: '10px 6px', }}>{t("student.schoolName")}</th>
                            <th style={{ width: 100, padding: '10px 6px', }}>{t("student.classRoom")}</th>
                            <th style={{ width: 50, padding: '10px 6px', }}>{t("student.gender")}</th>
                            <th style={{ width: 100, padding: '10px 6px', }}>{t("student.joined")}</th>
                    ...
                    <tbody>
                        {data.map((row, index) => (
                            <tr key={index}>
                                ...
                                <td>
                                    <Typography level="body-sm">{row.school_name}</Typography>
                                </td>
                                <td>
...
const StudentPage = ({
    ...
    useEffect(() => {
        ...
        if (createStatus == STATUS.SUCCESS) {
            ...
            dispatch(studentListRequest(accessToken, {"search": search}, page))
    ...
    useEffect(() => {
        ...
        dispatch(studentListRequest(accessToken, data, page))
    }, [page, search])
    ...
    const handleSubmit = (event) => {
        ...
        if (studentData.first_name) {
            ...
            // formData.append("email", "eksld.hdjj@gma.co.tz");
            // formData.append("password", "Parent@2025");
            formData.append("role", "student");
            formData.append("card_number",studentData.card_number)
            ...
            {listData.length > 0 ? <>
                ...
                >
                    ...
                    <Typography level="body-sm" mx="auto" textAlign={'center'} sx={{ display: { xs: 'flex', md: 'none' } }}>
                        Page {page} of {Math.ceil(totalStudent / ITEMS_PER_PAGE)}
                    </Typography>
                    <Box sx={{ flex: 1 }} />
                    ...
                    <Stack component='form' onSubmit={handleSubmit} gap={2} sx={{ mt: 2 }}>
                        ...
                        <Stack direction={{ xs: 'column', md: 'row' }} gap={2}>
                            ...
                            <FormControl sx={{ flex: 1 }} required>
                                ...
                                <Input type="text" name="school_name" value={studentData.school_name} onChange={handleChange} placeholder={t("init.placeholder") + t("student.schoolName")} />
                            </FormControl>
                        ...
                        <Stack direction={{ xs: 'column', md: 'row' }} gap={2}>
                            ...
                            <FormControl sx={{ flex: 1 }}>
                                <FormLabel>{t("student.card_number")}</FormLabel>
                                <Input type="text" name="card_number" value={studentData.card_number} placeholder={t("init.placeholder") + t("student.card_number")} onChange={handleChange} />
                            </FormControl>
                        </Stack>
...
const mapStateToProps = ({ auth, user }) => {
    ...
    const {
        ...
        studentListStatus: listStatus,
        studentListResult: listResult,
        studentListErrorMessage: listErrorMessage,
